...
テロップの位置は番組によって大きく違うため、設定ファイルを差し替えて使います。

| 設定ファイル | 想定レイアウト |
| --- | --- |
| `config.json` | ニュース番組型（画面**下部**に「所属・役職・氏名」の肩書きボックス） |
| `config.interview.json` | インタビュー／対談型（画面**左側**に「会社名 / 役職 / 氏名」が縦に並び、その下に経歴ボックス、下部中央に字幕） |

```bash
python main.py --url "https://www.youtube.com/watch?v=XXXX" --config config.interview.json
```

インタビュー型のプリセットは、経歴ボックス（長文）と画面下部の字幕（発話テロップ）を
クロップ範囲から除外してあります。これらを含めるとOCRのコストが増え、
経歴文から誤った会社名・部署名を拾う原因になります。
...
1. **ルールベース一次判定**: `pipeline/dictionaries.py` の辞書と正規表現で
   会社名（`〜株式会社` `〜市` `〜省` `〜銀行`、`ABC Inc.` `Global Tech Pte Ltd` …）、
   部署・役職（`〜部` `〜課` `課長補佐`、`Head of HR` `Managing Director` `CTO` …）、
...
実在の動画が使えない環境向けに、ffmpeg で生成した検証用動画2種類で Step 2〜6 を
通しで確認しています（OCRはローカル Tesseract で実施）。

### (a) ニュース番組型（854x480 / 54秒 / 背景が動く映像の下部にテロップ、同一人物が2回登場）
...
### (b) インタビュー型（1280x720 / 30秒 / 左側に会社名・役職・氏名、下に経歴ボックス、下部中央に字幕）

`config.interview.json` を使用。ゲストは10秒ごとに3人交代、字幕は2.5秒ごとに変化。

| 指標 | 結果 |
| --- | --- |
| 候補フレーム | 10枚 |
| OCRに投げたフレーム | **3枚**（字幕・経歴をクロップ外にしたため、テロップが変わった瞬間だけが残る） |
| 名寄せ後のレコード | 3件（3人と一致） |

```
アーチーズ株式会社   | Head of HR        | 和田 直子
株式会社サンプル物流 | 執行役員 CTO      | 山本 健太
Global Tech Pte Ltd | Managing Director | 佐々木 玲奈
```

いずれも会社名・部署名・氏名の分離は正しく、差異はOCR自体の誤認識（`太郎` → `太朗` など）のみでした。
